...
Единственное, это я не понимаю, почему на диаграме отображается связь между `users` и `wallets` как `one to many`, потому что у `wallets` ключ `user_id` является `PRIMARY` и ссылается на `id` в `users`, что уже должно гарантировать связь `one to one`
